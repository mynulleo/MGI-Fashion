...
-   Themes are located to `resources/views/themes` directory.

-   Every new theme will add to `resources/views/themes/{theme_directory_name}` directory.
...
-   To add a new theme, login to admin panel and create a theme from theme module.

-   The default theme will be active, when all themes are deactivated or no extra theme without default theme.

-   Please run the command in your project terminal: `php artisan sync-active-theme` or visit the URL: `${APP_URL}/admin/sync-active-theme`. If you visit URL, you must be logged in before visit this URL.

-   All build files and directories are stored in `public/build` directory.
-   For admin there are two directories into `build` directory: `admin` and `js`.
-   For theme/website there is one directory into `build` directory: `theme`.

-   Tips: when you may not run `npm` commands into your server, build locally and update `resources/build/theme` directory from `public/build/theme` directory. It's only for theme not admin (`public/build/theme` directory is a symblink). For admin, you may directory replace your server `public/build/admin` and `public/build/js` directories.

-   All routes about theme/website must be write into `routes/theme.php`.
...
In your index page , inside data state, update your this array into your table.routes.array state.
                ...
                array: [
                    {
                        ...
                        params: [
                            {
                                name: "id", // vue route parmas name
                                field: "id", // route params value from which field
                            },
                        ],
                        query: [
                            {
                                field: "is_show", // query param name
                                value: 1, // query param value
                            },
                        ],
                    },
...
<File
    ref="childRef"
    title="Logo (W:149px,H:200px)"
    field="data.logo"
    mime="img"
    req="true"
    fileClassName="data.logo"
/>
```
...
#### Custom FormData Create method

There are two methods available to create your custom formdata with vuejs state data where you can add files.

If you have any files, you have to add this file in your vuejs state like this:
your_file: $event.target.files[0];
...
```
 <!-------------- Custom Form data create --------------->
    this.createFormData(this.data)
  <!-------------- Custom Form data create --------------->
```

#### Safe Number (String to Number converter)

There is a method called safeNumber to convert from string to number where you may get a number (float/int).
This method ensure that you must be get number thouth it can be 0 if there is not possible to convert.
For example, '123' and '123.32' will be return '123' and 123.32 but '123abc' or 'abc23' will return 0.
...
-   Media validator is a wrapper for your module file/image.
-   First of all, create a new validator from Medial validator module from admin panel.
    Example:
...
-   Here `$root.media_validators?.logo?.min_height` is an example of media validator.

-   Then put these codes into created method:

```
this.getMediaValidators("SiteSetting");
```

-   Here `SiteSetting` is the name of your model.
-   In `submit` method:
...
-   Follow Site Setting module for full example about `Media Validator`.
