...
class FrontMenu extends BaseModel
{
    ...
    {
        return $this->childrenMenus()->with('allChildrenMenus', 'content')->oldest('sorting')->select(
            'id',
            'parent_id',
            'content_id',
            'title',
    ...
    public static function renderFrontMenu()
    {
        // Get all active header menus once
        static $menuItems = null;

        if ($menuItems === null) {
            $menuItems = FrontMenu::where('position', 'header')
                ->where('status', 'active')
                ->orderBy('sorting', 'asc')
                ->get()
                ->toArray();
    ...
    public static function buildMenuTree(array $menuItems, $parentId = null)
    {
        $branch = [];

        foreach ($menuItems as $menu) {
            if ($menu['parent_id'] == $parentId) {
                $children = Self::buildMenuTree($menuItems, $menu['id']);
                if ($children) {
