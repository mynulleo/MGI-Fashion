        ...
        <div class="position-relative">
            ...
            <div v-else class="d-grid gap-2 mt-2">
                <button v-if="data.block == 0" class="btn btn-danger text-white" data-bs-toggle="modal"
                    ...
                    <i class="fa fa-shield"></i> Block Organization
                </button>
                <button v-if="data.block == 1" class="btn btn-info text-white" data-bs-toggle="modal"
                    data-bs-target="#blockModal">
                    <i class="fa fa-shield"></i> Unblock Organization
                </button>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <template v-if="data.block == 0">
                            Are you sure you want to block <b>{{ data.organization_name }}</b>?
                        </template>
                        <template v-if="data.block == 1">
                            Are you sure you want to unblock <b>{{ data.organization_name }}</b>?
                        </template>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button v-if="data.block == 0" class="btn btn-info text-white"
                            @click="submitAction('block', { organization_id: data.id })"
                            data-bs-dismiss="modal">Confirm</button>
                        <button v-if="data.block == 1" class="btn btn-info text-white"
                            @click="submitAction('unblock', { organization_id: data.id })"
                            data-bs-dismiss="modal">Confirm</button>
                    </div>
                </div>
            </div>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select v-model="userForm.role_id" class="form-control">
                                    ...
                                    <option v-for="role in ($root.global.roles || [])" :key="role.value"
                                        :value="role.value">
                                        {{ role.name }}
                                    </option>
                                </select>
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <select v-model="passForm.id" class="form-control">
                                    ...
                                    <template v-for="(user, index) in data.organization_users">
...
export default {
    ...
    methods: {
        submitChangePassword() {
            ...
            axios.post('/organization/changepass', this.passForm)
                .then(res => {
                    this.$toast(
                        res.data.message,
                        'success'
        ...
        submitForm() {
            ...
            axios.post("organization-approved", this.dbForm)
                .then(() => {
                    this.$emit("submitted");
                })
                .catch((err) => {
        ...
        submitAction(action, data) {
            axios.post(`organization/${action}`, data).then(() => {
                this.$emit("submitted");
            });
        },
        fetchDBInfo() {
